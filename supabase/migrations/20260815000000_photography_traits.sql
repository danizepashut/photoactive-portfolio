-- פוטואקטיב, שדה "שלושה דברים שמאפיינים את הצילום שלי"
-- עלה תוך כדי סקירת עיצוב עמוד חמש: הוחלט להחליף מטא-דאטה טכנית מומצאת
-- (סרט/גישה/נושא) בתובנה אישית אמיתית של התלמיד, כשדה טופס אמיתי.

alter table public.students
  add column if not exists trait_1 text,
  add column if not exists trait_2 text,
  add column if not exists trait_3 text;
...
begin
  ...
  if coalesce(trim(v_student.display_name), '') = ''
     ...
     or coalesce(trim(v_student.trait_1), '') = ''
     or coalesce(trim(v_student.trait_2), '') = ''
     or coalesce(trim(v_student.trait_3), '') = ''
  then
